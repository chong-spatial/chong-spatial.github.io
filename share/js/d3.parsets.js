...
// 
     while (d.parent && d.hasOwnProperty('source') ) {
        if (d.name) path.unshift(d.dimension+'=<b>'+dataLabels[d.dimension][d.name]+'</b>');
        d = d.parent;
      }

    
    return '<b>Edible='+lab + '</b>:<br>' + path.join(" → ") + "<br>" + comma(count) + " (" + percent(count / d.count) + ")";
  }

  function defaultCategoryTooltip(d) {
    return d.dimension.name+'=<b>'+dataLabels[d.dimension.name][d.name] +'</b><br>'+ comma(d.count) + " (" + percent(d.count / d.dimension.count) + ")";
  }
