     ...
     _render: function() {
       ...
       if(this.svg_type == 'path'){
        ...
        featureg.each(function(d){
          ...
          var vorVertices = d3.range(7).map(function(d) {
          ...
          //store the voronoi data in geographic coordinates
          //d.vorScrCells = vorCells;
          d.vorGeoData0 = vorPathData0.map(function(c){ return c.map(function(d){ if(typeof d != 'undefined'){ var wtg = esri.geometry.webMercatorToGeographic(map.toMap(new esri.geometry.ScreenPoint(d[0],d[1]))); return [wtg.x, wtg.y]}})});
          d.vorGeoData1 = vorPathData1.map(function(c){ return c.map(function(d){ if(typeof d != 'undefined'){ var wtg = esri.geometry.webMercatorToGeographic(map.toMap(new esri.geometry.ScreenPoint(d[0],d[1]))); return [wtg.x, wtg.y]}})});
         

          var vorPath = d3.select(this).append('g').selectAll('.voronoi').data(vorPathData0);
          vorPath.enter().append('path').attr('class', 'voronoi');
          vorPath.exit().remove();

          vorPath.each(function(d, i){  
            //console.log(vorPathData1[i]);
            //d.vor0GeoCoords = d.map(function(r){ var wtg = esri.geometry.webMercatorToGeographic(map.toMap(new esri.geometry.ScreenPoint(r[0],r[1]))); return [wtg.x, wtg.y]}) 
         ...
         */
         ...
         loop();
          ...
          function generateBestVoronoiVertices(){
            var iter = 0;
            while(iter<10){ // max exe count, 10, 100
     ...
     redraw: function() {
       ...
       if (this.svg_type != 'path') {
        ...
        g.each(function(d){
          ...
          var statetweening = d3.select(this).select('.tweenState').attr("d", d0)
           

          
          var v0 = d.vorGeoData0.slice(),
              v1 = d.vorGeoData1.slice();
          var vorPathData0 = v0.map(function(r){ return r.map(function(v){ return self._project_to_screen(v) })}),
              vorPathData1 = v1.map(function(r){ return r.map(function(v){ return self._project_to_screen(v) })});
