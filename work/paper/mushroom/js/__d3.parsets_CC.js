...
(function() {
  d3.parsets = function() {
    ...
    var highlightedRibbons = [], highlightCates = [], highlightItems = [];
      ...
      selection.each(function(data, i) {
        ...
        function updateDimensions() {
          ...
          if(yesLabelGoesFirst) {
            ...
            if(classCates[0].name == 'D'){
              var tmp = Object.assign({}, classCates[0]);
    ...
    parsets.toggleRibbons = function(items){
      ...
      if (!~highlightItems.indexOf(itemKey)) { // new to highlight    
        highlightItems.push(itemKey);        
      
      } else {
        var idx = highlightItems.indexOf(itemKey);
        highlightItems.splice(idx, 1); 
     
      } 

      var highlightDims =  [],
          highDimCate = {};
      highlightItems.forEach(function(h){
        var dc_ = h.split('\0');
    ...
    parsets.toggleCategoriesBG = function(dimCatesDict) { 
      for(var dim in dimCatesDict){
        ...
        if (!~highlightCates.indexOf(dimCateName)) { // new to highlight    
          highlightCates.push(dimCateName);
        
        
        } else { // unhighlight
          var idx = highlightCates.indexOf(dimCateName);
          highlightCates.splice(idx, 1); 
                ...
                .on("drag", function(d) {
                  ...
                  for (var i = 1; i < newDimensions.length; i++) {
                    if (height * newDimensions[i].y < height * newDimensions[i - 1].y) {
                      ...
                      ordinal.domain([]).range(d3.range(parsets.classDimension().categories.length));
                      nodes = layout(tree = buildTree({children: {}, count: 0}, data, dimensionNames, value_), newDimensions, ordinal);
                      total = getTotal(newDimensions);
                      g.selectAll(".ribbon, .ribbon-mouse").selectAll("path").remove();
                      parsets.updateRibbons();
                      updateCategories(dimension);
    ...
    parsets.updateRuleCategoryClass = function(ruleDimCatesDict) {     
      ...
      g.selectAll('.ribbon, .ribbon-mouse').selectAll("path").filter(function(d){         
        ...
        if(ruleUnrelatedCateIsShown){
          ...
          //d3.select(this).classed('ruleNotRelatedCateShown', true);

          
        } else {
          //d3.select(this).classed('ruleNotRelatedCateShown', false);
