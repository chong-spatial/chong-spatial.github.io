...
var colorSupportGreen = d3.scale.linear()
      ...
      .range([d3.rgb("#7bb779"), d3.rgb('#0a6d06')]);

var colorSupportRed = d3.scale.linear()
      .interpolate(d3.interpolateHcl)
      .range([d3.rgb("#eb7272"), d3.rgb('#eb1515')]);

var c = function(cls, supp){ return cls == "P1" ? colorSupportGreen(supp) : colorSupportRed(supp)};
...
d3.ruleGrid = function() {
  ...
  function dataProcess(newData){
    ...
    // {name: Class, categories: [{name: 2nd, rules: {p1: [], p2: []}}]}
    ...
    // var test = aggregationSorting(dimensions, 'P1')
    // console.log(test)
    ...
    orderFunctions = {
        ...
        yIndex: newData.filter(function(d){ return d.cls == 'P1'}).sort(function(a, b) { return d3.ascending(a.id, b.id)}).concat(newData.filter(function(d){ return d.cls == 'P2'}).sort(function(a, b) { return d3.ascending(a.id, b.id)}) ).map(function(d){ return d.id }),
        ...
        itemNameInCls: function(dimName){ 
          ...
          var rulesLabYes = newData.filter(function(d){ return d.cls == 'P1'}),
              rulesLabNo = newData.filter(function(d){ return d.cls == 'P2'});
        ...
        ruleSuppInCls: newData.filter(function(d){ return d.cls == 'P1'}).sort(function(a, b) { return d3.descending(a.m[0], b.m[0])}).concat(newData.filter(function(d){ return d.cls == 'P2'}).sort(function(a, b) { return d3.descending(a.m[0], b.m[0])}) ).map(function(d){ return d.id }),
        
        ruleConfInCls: newData.filter(function(d){ return d.cls == 'P1'}).sort(function(a, b) { return d3.descending(a.m[1], b.m[1])}).concat(newData.filter(function(d){ return d.cls == 'P2'}).sort(function(a, b) { return d3.descending(a.m[1], b.m[1])}) ).map(function(d){ return d.id }),
        ...
        aggregationYes: aggregationSorting(dimensions, 'P1').map(function(d){ return d.index }),
        aggregationNo: aggregationSorting(dimensions, 'P2').map(function(d){ return d.index }),
        ruleAggYes: function(){
          var aggDims = aggregationSorting(dimensions, 'P1');
          var yesRules = [];
          for(var i=0; i< aggDims.length; i++){
            var cates = aggDims[i].categories;
            for(var j = 0; j<cates.length; j++){
              if(cates[j].rules.hasOwnProperty('P1')){
                yesRules = yesRules.concat(cates[j].rules['P1'])
              }
            }
          }
          // may have duplicates
          ...
          return uniqueYesRule.concat(newData.filter(function(d){ return d.cls == 'P2'}).sort(function(a, b) { return d3.ascending(a.id, b.id)}) ).map(function(d){ return d.id })

        },
        ruleAggNo: function(){
          var aggDims = aggregationSorting(dimensions, 'P2');
          var noRules = [];
          for(var i=0; i< aggDims.length; i++){
            var cates = aggDims[i].categories;
            for(var j = 0; j<cates.length; j++){
              if(cates[j].rules.hasOwnProperty('P2')){
                noRules = noRules.concat(cates[j].rules['P2'])
              }
            }
          }
          // may have duplicates
          ...
          return newData.filter(function(d){ return d.cls == 'P1'}).sort(function(a, b) { return d3.ascending(a.id, b.id)}).concat(uniqueNoRule).map(function(d){ return d.id })
  ...
  function similarityOrder(by) {
    ...
    if(by == 'mixCloseness') {
      ...
      for(var i = 0; i < matrix.length; i++){
        var row = matrix[i];
        if(row.find(function(a){ return a.z !=0 }).rule.cls == 'P1'){
            for(var j = 0; j < row.length; j++){
            var cell = row[j];
            binaryMatrix[j][i] = cell.z == 0 ? 0 : 1;
          }
      ...
      for(var i = 0; i < matrix.length; i++){
        var row = matrix[i];
        if(row.find(function(a){ return a.z !=0 }).rule.cls == 'P2'){
            for(var j = 0; j < row.length; j++){
            var cell = row[j];
            binaryMatrix[j][i] = cell.z == 0 ? 0 : 1;
          }
  ...
  exports.redraw = function(){    
    ...
    colorSupportGreen.domain(d3.extent(rData.filter(function(d){return d.cls === 'P1'}), function(d){ return d.m[0]}))
    colorSupportRed.domain(d3.extent(rData.filter(function(d){return d.cls === 'P2'}), function(d){ return d.m[0]}))
    ...
    rowEnter.append('rect')
      ...
      .attr("height", y.rangeBand())   
        .classed('rowBgGreen', function(d){ 
          return d.find(function(a){ return a.z !=0 }).rule.cls === 'P1'})
        .classed('rowBgRed', function(d){ return d.find(function(a){ return a.z !=0 }).rule.cls === 'P2'})
        //.style('display', 'none')
    ...
    function reDrawRow(row) {  
      ...
      cell.enter().append("rect")
          ...
          .style("fill", function(d) { return c(d.rule.cls, d.rule.m[0]); })
