/**
* Multi-line graph for time series
* @author: C. Zhang
* adapted from NVD3
*/
...
var linegraph = (function() {
	...
	var config = {
		// default setting
		tFormat = d3.time.format("%Y%m%d").parse;
	}
	...
	// func() // call private func
	//} 

	var configFunc = function(tData, containerId, w, h, lineColor) {
	...
	*/
	...
	function highlightRose(d,id){
		if (document.getElementById(id).nodeName=='line'){
			...
			d3.select("#path"+id.substr(id.indexOf('_'))).style("stroke","black");
			...
			d3.select("#line"+id.substr(id.indexOf('_'))).style("stroke","black");
		}else{//path
			d3.select("#"+id).style("stroke-width",2);
			d3.select("#"+id).style("stroke","black");
